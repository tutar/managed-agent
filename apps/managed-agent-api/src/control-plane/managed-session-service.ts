...
type EventPublisher = {
  open(response: ServerResponse<IncomingMessage>): void
  publish(
    response: ServerResponse<IncomingMessage>,
    event: SessionRunEvent | { type: string; data: unknown },
  ): void
...
const resolveRequestedModel = (model: string | undefined) => {
  const configuredDefaultModel =
    process.env.MANAGED_AGENT_DEFAULT_MODEL ?? "managed-agent-local"
...
const consumeRun = async ({
  ...
  while (true) {
    const next = await Promise.race([
      iterator.next(),
      cancellationState.waitForCancellation(),
    ])
    ...
    const iteratorResult = next as IteratorResult<
      SessionRunEvent,
      SessionRunCompletion
    >
    ...
    const isProcessEvent =
      event.type.startsWith("process.") || event.type.startsWith("action.")
    ...
    if (
      ...
      processContent.push({
        ...
        ...(toolCallStatus === "completed" && event.data.result
          ? { result: event.data.result }
          : {}),
        ...(toolCallStatus === "error" && event.data.error
...
export const createManagedSessionService = ({
  ...
  return {
    ...
    async createSession({
      ...
      const assistantEntryId = createAssistantEntry(
        processEntry.id,
        "pending",
        undefined,
        now,
      ...
      try {
        ...
        eventPublisher.publish(response, {
          ...
          data: {
            ...
            parentId: processEntry.id,
            code:
              error instanceof HarnessWorkerExecutionError
                ? error.code
    ...
    async submitMessage({
      ...
      const userEntry = createUserEntry(
        request.input,
        getLastEntryId(session.entries),
        new Date().toISOString(),
      )
      ...
      try {
        ...
        await sessionRepository.updateSession({
          ...
          status: "idle",
          piSessionFile:
            runResult.completion.piSessionFile ?? session.piSessionFile,
          updatedAt: new Date().toISOString(),
          entries: [
            ...pendingSession.entries,
            {
    ...
    async getSessionStatus(sessionId: string) {
      ...
      return activeSessionRegistry.isActive(sessionId)
        ? "running"
        : session.status
    },
    ...
    async archiveSession(sessionId: string) {
      ...
      if (
        activeSessionRegistry.isActive(sessionId) ||
