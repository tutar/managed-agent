...
const isManagedTextItem = (value: unknown): value is ManagedTextItem => {
	return hasObjectShape(value) && value.type === "text" && typeof value.text === "string";
};

const isManagedToolCallItem = (value: unknown): value is ManagedToolCallItem => {
	return (
		hasObjectShape(value) &&
		value.type === "tool_call" &&
		typeof value.toolCallId === "string" &&
		typeof value.toolName === "string" &&
		(value.status === "started" || value.status === "completed" || value.status === "error")
	);
};
...
const isPiContentItem = (value: unknown): value is PiContentItem => {
	...
	if ((value.type === "image" || value.type === "video") && typeof value.url === "string") {
		return true;
	}
...
const isPiMessageRecord = (value: unknown): value is Extract<PiSessionJsonlRecord, { type: "message" }> => {
	return (
		hasObjectShape(value) &&
		value.type === "message" &&
...
const isManagedTranscriptRecord = (value: unknown): value is ManagedTranscriptRecord => {
	return (
		hasObjectShape(value) &&
		((value.type === "managed_session" && typeof value.sessionId === "string" && value.runtime === "mock") ||
			(value.type === "managed_entry" &&
				typeof value.sessionId === "string" &&
				"entry" in value &&
				hasObjectShape(value.entry) &&
				typeof value.entry.id === "string" &&
				(value.entry.parentId === null || typeof value.entry.parentId === "string") &&
...
const toCreatedAt = (record: Extract<PiSessionJsonlRecord, { type: "message" }>) => {
...
const toAssistantPreamble = (content: unknown[]) => {
	const firstToolCallIndex = content.findIndex((item) => isPiContentItem(item) && item.type === "toolCall");
	const contentBeforeToolCalls = firstToolCallIndex >= 0 ? content.slice(0, firstToolCallIndex) : content;
		...
		return [
			{
				...
				...(item.arguments !== undefined ? { arguments: JSON.stringify(item.arguments) } : {}),
			},
		];
	});
};
...
const resolveTranscriptPath = (piSessionFile: string, transcriptsRoot: string) => {
...
export const createPiFileTranscriptReader = ({
	...
	return {
		async readSessionEntries({ piSessionFile }: TranscriptReadInput): Promise<SessionEntry[]> {
			if (!piSessionFile) {
				return [];
			}
			...
			for (const line of lines) {
				...
				if (isManagedTranscriptRecord(parsed) && parsed.type === "managed_entry") {
				...
				if (parsed.message.role === "toolResult") {
					...
					const processEntryId = processEntryIdByPiMessageId.get(parsed.parentId);
