...
const createCompatibleModelDefinition = (provider: LlmProviderRuntimeConfig) => {
	const availableModels =
		provider.availableModels && provider.availableModels.length > 0
			...
			: [{ modelId: provider.modelId, displayName: provider.modelId, supportsReasoning: provider.supportsReasoning }];
...
const applyRuntimeProviderConfig = ({
	...
	if (llmProvider.authMode === "api_key" && llmProvider.apiKey) {
		authStorage.set(llmProvider.runtimeProviderId, { type: "api_key", key: llmProvider.apiKey });
	}
	...
	if (llmProvider.usesBuiltInProvider && !llmProvider.baseUrl && (!llmProvider.headers || Object.keys(llmProvider.headers).length === 0)) {
		return;
	}
	modelRegistry.registerProvider(llmProvider.runtimeProviderId, {
		...
		...(llmProvider.usesBuiltInProvider ? {} : { models: createCompatibleModelDefinition(llmProvider) }),
	});
};
...
export const createPiAdapter = (): AgentAdapter => ({
	...
	async *run(input: HarnessInput) {
		...
		const sessionDir = input.sessionDir ?? join(cwd, ".managed-agent", "transcripts", "pi-sessions");
		...
		if (input.llmProvider) {
			applyRuntimeProviderConfig({ authStorage, modelRegistry, llmProvider: input.llmProvider });
		}
		...
		const sessionPath = input.piSessionFile
			? isAbsolute(input.piSessionFile) ? input.piSessionFile : join(sessionRoot, input.piSessionFile)
			: undefined;
		...
		const unsubscribe = session.subscribe((event: AgentSessionEvent) => {
			if (event.type === "message_update" && event.assistantMessageEvent.type === "text_delta") {
				...
				if (delta && delta.length > 0) bufferedEvents.push({ type: "text_delta", text: delta });
			}
