...
const writeSseEvent = (response: ServerResponse<IncomingMessage>, eventName: string, data: unknown) => {
...
const sendJson = (response: ServerResponse<IncomingMessage>, statusCode: number, body: unknown) => {
 ...
 */
export const createInternalRunHttpHandler = ({ executor }: { executor: SessionExecutor }) => {
	return async (request: IncomingMessage, response: ServerResponse<IncomingMessage>) => {
		try {
			...
			if (request.method === "POST" && url.pathname === "/internal/session-runs") {
